...
def evaluate_critic_loss(model, batch):
    ...
    q_loss = 0.5 * (mask * (q_target * (1 - config.gamma) - q_tm1).pow(2)).sum(1).mean() + 1e-3 * l2_q
...
@torch.no_grad()
def _sample_trajectory(state, env, actor, trajectory, writer_queue, trajectory_len, proc_id):
