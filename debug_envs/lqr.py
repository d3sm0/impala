...
class Lqg(gym.core.Env):
    ...
    def __init__(self):
        ...
        np.random.seed(0)
        self.initial_state = np.array(np.random.normal(size=(2,)), dtype=np.float32)
        # self.A = np.array([[1., 0.], [0., 1.]], dtype=np.float32)
        self.B = np.array([[0.3], [-0.3]], dtype=np.float32)

        self.A = np.array([[0.9, 0.4], [-0.4, 0]], dtype=np.float32)
        # Cost
        self.Q = 0.1 * np.eye(state_dim)
        self.R = np.array([[1.]], dtype=np.float32)

        self.K = np.array([[3.34826643, -3.34826643]])
        self.S = np.array([[5.58044406, -5.58044406], [-5.58044406, 5.58044406]])
        self.state = None
    ...
    def step(self, action: np.ndarray):
        ...
        # action = action * (1 + 1) - 1
        # x = z * (b-a) + a
        # action = torch.from_numpy(action).to(np.float32)
        reward = self.reward_fn(self.state, action)
        self.state = self.dynamics_fn(self.state, action)
        done = np.array(1.) if np.abs(action).sum() > 1e1 else np.array(0.)
        done = done if self.t < 200 else np.array(1.)
        self.t += 1
